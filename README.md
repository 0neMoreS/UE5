...
- 可行的解决方案：
	...
	2. 在逆光区（用 N dot L 判断）降低 SurfaceThickness - 试了，效果不好，保留了 artifact 的同时还几乎让 Contact Shadow 无效（因为 SurfaceThickness 降低了）

    3. 由于 thickness 信息无法还原，我们在采样前无法模糊样本信号，所以只能考虑在采样后的逆光区做阴影 blend

    4. 传统的 ScreenSpaceShadowRayCast 实现是沿着光源方向间隔几个像素采一次深度缓冲，采样频率更低，或许可以减轻 artifact？ - 尝试了关闭 UseHalfPixelOffset，降低采样率，稍微有点效果，但是没有根除

    5. 降采样机制？不是，Bend 分支完全没走降采样，ScreenSpaceShadows 的 DownsampleFactor 不影响 CS 中的 Bend 分支
